...
prediction_df <-
  augment(lm(e_rate ~ e_rate_lag, data = training_set),
          newdata = na.omit(testing_set)) |>
...
for (i in 2:(nrow(prediction_df))) {
  # iteration for calculating the balance

  p_balance <- last(balance_df$balance)
  p_currency <- last(balance_df$currency)

  if (prediction_df$.fitted[i] < l_bound) {
    # euro gets cheaper > buy huf
    balance <-
      ifelse(p_currency == "eur",
             p_balance * prediction_df$close[i - 1],
             p_balance)
    currency <- "huf"

  } else if (prediction_df$.fitted[i] > u_bound) {
    # euro gets more expensive > buy euro
    balance <-
      ifelse(p_currency == "huf",
             p_balance / prediction_df$close[i - 1],
             p_balance)
    currency <- "eur"
  } else {
...
bind_cols(prediction_df, balance_df) |>
  mutate(balance = ifelse(currency == "huf", balance / close, balance)) |>
  ggplot() +
